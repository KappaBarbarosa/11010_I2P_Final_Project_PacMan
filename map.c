...
static void draw_power1_bean(Map* M, const int row, const int col);
static void draw_power2_bean(Map* M, const int row, const int col);
static void draw_power3_bean(Map* M, const int row, const int col);
static void draw_timer_bean(Map* M, const int row, const int col);
static void draw_portal_bean(Map* M, const int row, const int col);
int sposition[5][2];

ALLEGRO_BITMAP* timer_pitcure=NULL;
ALLEGRO_BITMAP* portal_pitcure;
const char* nthu_map[] = {
	"#####################################",
	"#..................###.........#####",
	"#.####.###########.....#######.....#",
...
Map* create_map(const char * filepath) {
	...
	for (int i = 0; i < M->row_num; i++) {
		for (int j = 0; j < M->col_num; j++) {
			...
			case '.':
				...
				break;
			case 'p':
				portal_matrix[pcount][0]= i  ;
				portal_matrix[pcount][1] = j;
				pcount++;
				break;
			default:
				break;
	...
	for(int i=0;i<5;i++) fscanf(pFile,"%d %d",&sposition[i][0],&sposition[i][1]);
	M->beansNum = M->beansCount;
	timer_pitcure = load_bitmap("Assets/clock.png");
	portal_pitcure = load_bitmap("Assets/portal_1.png");
	if(!timer_pitcure) printf("failed lpad c;ock\n");
	return M;
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				...
				case '.':
					...
					break;
				case 'T':
					draw_timer_bean(M, row, col);
					break;
				case 'p':
					draw_portal_bean(M, row, col);
					break;
				case 'A':
					if(bosslife==3)draw_power1_bean(M, row, col);
					break;
				case 'C':
					if(bosslife==2)draw_power2_bean(M, row, col);
					break;
				case 'D':
					if(bosslife==1)draw_power3_bean(M, row, col);
					break;
				default:
					break;
			}
...
static void draw_timer_bean(Map* M, const int row, const int col) {
	al_draw_scaled_bitmap(timer_pitcure,0,0,15,15,(map_offset_x + col * block_width + block_width/2.0)-15,( map_offset_y + row * block_height + block_height / 2.0)-15,30,30,0);
}
static void draw_portal_bean(Map* M, const int row, const int col){
	al_draw_scaled_bitmap(portal_pitcure,0,0,235,400,(map_offset_x + col * block_width + block_width/2.0)-15,( map_offset_y + row * block_height + block_height / 2.0)-15,25,30,0);
}
static void draw_power1_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(0, 255, 255));
}
static void draw_power2_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(255, 255, 0));
}
static void draw_power3_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(181, 79, 227));
}
...
Directions boss_path_direc(Map* M, int startGridx, int startGridy, int endGridx, int endGridy) {
static int8_t queue_x[QUEUE_SIZE];
static int8_t queue_y[QUEUE_SIZE];
static	uint16_t front;
static	uint16_t end;
