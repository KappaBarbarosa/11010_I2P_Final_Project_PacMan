...
ALLEGRO_SAMPLE* clock_sound = NULL;
...
void shared_init(void) {
	clock_sound = load_audio("Assets/Music/clockbgm.mp3");
	menuFont = load_font("Assets/Minecraft.ttf", fontSize);
	winFont_s = load_font("Assets/Minecraft.ttf", 40);
	winFont = load_font("Assets/cool.otf", winsize);
	...
	ghost_sound = load_audio("Assets/Music/pacman_eatghost.ogg");
}

void shared_destroy(void) {
	al_destroy_font(winFont_s);
	al_destroy_font(winFont);
	al_destroy_font(menuFont);
	al_destroy_sample(themeMusic);
	al_destroy_sample(clock_sound);
