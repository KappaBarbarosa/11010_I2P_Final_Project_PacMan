...
void game_create() {
	fp = fopen("Assets/score_record.txt","r");
	for(int i=0;i<10;i++) fscanf(fp,"%d %d",&srecord[i].score,&srecord[i].time);
	fclose(fp);
...
static void game_start_event_loop(void) {
	...
	while (!gameDone) {
		...
		if (event.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
			fp = fopen("Assets/score_record.txt","w");
			for(int i=0;i<10;i++) fprintf(fp,"%d %d\n",srecord[i].score,srecord[i].time);
			fclose(fp);
